...
def createTree(dataSet, leafType=regLeaf, errType=regErr, ops=(1, 4)):
    """
    createTree(获取回归树)
    递归函数：如果构建的是回归树，该模型是一个常数，如果是模型树，其模型师一个线性方程。
    Args:
        dataSet: 加载的原始数据集
        leafType: 建立叶子点的函数
        errType: 误差计算函数
...
if __name__ == "__main__":
    ...
    # myDat = loadDataSet('../data/ch09/ex0.txt')
    # print('myDat=', myDat)
    # myMat = np.mat(myDat)
    # print('myMat=', myMat)
    ...
    myDat = loadDataSet('../data/ch09/ex2.txt')
    myMat = np.mat(myDat)
    ...
    # ------
    # 模型树求解
    # myDat = loadDataSet('../data/ch09/data4.txt')
    # myMat = np.mat(myDat)
    # myTree = createTree(myMat, modelLeaf, modelErr)
    # print(myTree)

    # 回归树 VS 模型树 VS 线性回归
    trainMat = np.mat(loadDataSet('../data/ch09/bikeSpeedVsIq_train.txt'))
    testMat = np.mat(loadDataSet('../data/ch09/bikeSpeedVsIq_test.txt'))
    ...
    print("回归树:", np.corrcoef(yHat1, testMat[:, 1], rowvar=False)[0, 1])

    # 模型树
    myTree2 = createTree(trainMat, modelLeaf, modelErr, ops=(1, 20))
    yHat2 = createForeCast(myTree2, testMat[:, 0], modelTreeEval)
    print(myTree2)
    print("模型树:", np.corrcoef(yHat2, testMat[:, 1], rowvar=False)[0, 1])
